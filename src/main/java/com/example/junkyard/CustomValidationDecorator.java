...
public class CustomValidationDecorator extends BaseValidationDecorator {
    ...
    public void insideLabel(Field field, Element element) {
        final String testFieldClassName = TextField.class.getName();

        if (field == null) {
            return;
        }

        if (!field.getClass().getName().equals(testFieldClassName)) {
            if (field.isRequired()) {
        ...
        if (inError(field)) {
           ...
           element.getContainer().addClassName("red");
        }
    }
    ...
    public void insideField(Field field) {
        ...
        if (inError(field)) {
            markupWriter.getElement().addClassName("medium input-text red");
            markupWriter.getElement().getContainer().addClassName("red");
        }
        else {
            markupWriter.getElement().addClassName("medium input-text");
        }
    }
